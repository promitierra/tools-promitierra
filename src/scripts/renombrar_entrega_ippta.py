...
def patron_renombrado_entrega(nombre_archivo: str) -> str:
    ...
    # Si el archivo ya tiene _IPPTA, extraemos el ID
    if "_IPPTA" in nombre_base:
        id_numero = nombre_base.split('_')[0]
        if id_numero.isdigit():
            return f"{id_numero}_ENTREGA_IPPTA{extension}"
...
def main():
    ...
    # Ruta del directorio (ajustar según sea necesario)
    directorio = r"C:\Users\mmluf\Fundación ProMITIERRA\TERRITORIAL EPSEA AMDELCA-Territorial Zona Sur - Documentos\2_OPERACION\CONSOLIDADO ENGREGA IPPTA´s & PLANEADORES\CONSOLIDADO FORMATO ENTREGA IPPTA - CURILLO - YENNY PAOLA MOLANO"
