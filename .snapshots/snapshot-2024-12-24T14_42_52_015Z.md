...
│   │   ├── __init__.py
│   │   ├── imagen_a_pdf.py  # Procesador principal
│   │   ├── image_processor.py
│   │   └── text_normalizer.py
...
- **core/**: Lógica principal
  - `imagen_a_pdf.py`: Procesador principal de imágenes a PDF
  - `image_processor.py`: Utilidades de procesamiento de imágenes
  - `text_normalizer.py`: Normalización de texto para nombres de carpetas
- **gui/**: Interfaz gráfica de usuario
- **utils/**: Utilidades y funciones auxiliares

#### Herramientas (`build_tools/`)
- **specs/**: Configuraciones de PyInstaller
- **icons/**: Recursos gráficos
- **scripts/**: Scripts de automatización
  - `build_exe.py`: Generación de ejecutables
  - `create_icon.py`: Creación de iconos

#### Pruebas (`tests/`)
- **data/**: Datos para pruebas
  - `input/`: Archivos de entrada (imágenes, Excel)
  - `expected/`: Resultados esperados
- **test_cases/**: Pruebas unitarias y de integración
- **output/**: Resultados generados por las pruebas
...
python build_exe.py ../specs/imagen_a_pdf.spec [versión]
```
...
```bash
python -m pytest tests.test_cases.test_text_normalizer
```
...
3. Ejecutar pruebas y asegurar que pasan
...
```
"""
Módulo para normalización de texto y nombres de carpetas.
"""
    ...
    def normalize_text(text):
        ...
        if len(parts) == 1:
            ...
            if not parts[0].strip():
                name = ' '.join(normalize_chars(' '.join(parts[1:]).split())).upper()
                return f"- {name}"
            # Si no hay guion, tratar todo como nombre
            if len(parts) == 1:
                return f"- {normalize_chars(' '.join(parts[0].split())).upper()}"
            # El primer elemento es el ID completo
            id_part = ''.join(parts[0].split())  # Eliminar espacios en el ID
            name = normalize_chars(' '.join(' '.join(parts[1:]).split())).upper()
